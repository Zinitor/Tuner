...
def callback(indata, frames, time, status):
    ...
    if any(indata):
        ...
        if callback.noteBuffer.count(callback.noteBuffer[0]) == len(callback.noteBuffer):
            bottomText["text"] = f"{closest_note} {max_freq}/{closest_pitch}"
            global progress_value
            progress_value = abs(max_freq - closest_pitch) * 10 + 50
            progressbar["value"] = progress_value
            if progress_value % 50 > 10:
                progressbar["style"] = "red.Horizontal.TProgressbar"
            else:
                progressbar["style"] = "green.Horizontal.TProgressbar"

        else:
            bottomText["text"] = closest_note

    else:
        bottomText["text"] = "no input"
...
def start_stop_audio_processing():
    global audio_processing_thread, is_recording
    ...
    if is_recording:
        ...
        audio_processing_thread = threading.Thread(target=update_progress)
        audio_processing_thread.daemon = True  # This makes the thread terminate when the main program exits
...
root.title("Tuner window")
root.minsize(200, 200)  # width, height
root.geometry("300x300+50+50")

labelText = tk.Label(root, text="This text should be on top")
labelText.pack()

progress_value = 50  # Initialize progress value

progressbar = ttk.Progressbar(root, orient="horizontal", mode="indeterminate", value=progress_value,
                              style='yellow.Horizontal.TProgressbar')
progressbar.place(relx=.5, rely=.5, anchor="center")
# progressbar.pack(fill="x", padx=6, pady=6)

bottomText = tk.Label(root, text="E")
bottomText.pack()

button = tk.Button(root, text="Record", command=start_stop_audio_processing)
button.pack()
